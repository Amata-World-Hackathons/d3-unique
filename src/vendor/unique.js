...
const nesting = {
  ...
  tokenIdToAddress(collectionId, tokenId) {
    return this.toChecksumAddress(
      `0xf8238ccfff8ed887463fd5e0${collectionId
        .toString(16)
...
class UniqueUtil {
  ...
  static isTokenTransferSuccess(
    ...
    const normalizeAddress = (address) => {
      ...
      if (obj.substrate)
        return { Substrate: this.normalizeSubstrateAddress(obj.substrate) };
...
class ChainHelperBase {
  ...
  async connect(wsEndpoint, listeners) {
    ...
    const { api, network } = await this.constructor.createConnection(
      wsEndpoint,
      listeners,
      this.forcedNetwork
    );
  ...
  static async createConnection(wsEndpoint, listeners, network) {
    const supportedRPC = {
      opal: {
        unique: require("@unique-nft/opal-testnet-types/definitions").unique
          .rpc,
      },
      quartz: {
        unique: require("@unique-nft/quartz-mainnet-types/definitions").unique
          .rpc,
      },
      unique: {
        unique: require("@unique-nft/unique-mainnet-types/definitions").unique
          .rpc,
      },
    };
    if (!supportedRPC.hasOwnProperty(network))
      network = await this.detectNetworkByWsEndpoint(wsEndpoint);
  ...
  signTransaction(sender, transaction, label = "transaction") {
    return new Promise(async (resolve, reject) => {
      try {
        let unsub = await transaction.signAndSend(
          sender.address,
            ...
            if (status === this.transactionStatus.SUCCESS) {
              ...
              if (result.hasOwnProperty("dispatchError")) {
                ...
                if (dispatchError.isModule) {
                  ...
                  const errorMeta =
                    dispatchError.registry.findMetaError(modErr);
...
class UniqueHelper extends ChainHelperBase {
  ...
  async transferBalanceToSubstrateAccount(
    ...
    let isSuccess =
      this.util.normalizeSubstrateAddress(
  ...
  async getCollection(collectionId) {
    ...
    let humanCollection = collection.toHuman(),
      collectionData = {
        id: collectionId,
        name: null,
        description: null,
  ...
  async getTokenChildren(collectionId, tokenId, blockHashAt) {
    ...
    if (typeof blockHashAt === "undefined") {
      ...
      children = await this.api.rpc.unique.tokenChildren(
        collectionId,
        tokenId,
        blockHashAt
      );
  ...
  async transferNFTToken(
    ...
    return this.util.isTokenTransferSuccess(
      ...
      collectionId,
      tokenId,
      { Substrate: typeof signer === "string" ? signer : signer.address },
      addressObj
    );
  }
  ...
  async mintNFTCollection(
    ...
    for (let key of ["name", "description", "tokenPrefix"]) {
      if (typeof collectionOptions[key] === "string")
        collectionOptions[key] = this.util.str2vec(collectionOptions[key]);
    }
  ...
  async confirmNFTCollectionSponsorship(
    ...
    try {
      ...
      if (e.status === this.util.transactionStatus.FAIL)
...
class UniqueNFTCollection {
  ...
  async getTokenChildren(tokenId, blockHashAt) {
    return await this.uniqueHelper.getTokenChildren(
      this.collectionId,
      tokenId,
      blockHashAt
    );
